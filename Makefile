...
build: check-variables
	if [ -n "${DOCKER_COMPOSE_PATH}" ]; then mkdir -p files/compose/${IMAGE_NAME}; cp -r ${DOCKER_COMPOSE_PATH}/* files/compose/${IMAGE_NAME}/; fi; \
	packer build -var 'project_id=${PROJECT}' -var 'image_name=${IMAGE_NAME}' packer.json; \
	if [ -n "${DOCKER_COMPOSE_PATH}" ]; then rm -rf files/compose/${IMAGE_NAME}; fi;
